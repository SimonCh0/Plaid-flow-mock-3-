...
export default function OrbitPrototype() {
  ...
  const SourcesScreen = () => {
    ...
    return (
      <div style={{
        ...
        {(connectedSources.bank || connectedSources.email) && (
          <div style={{
            ...
            {connectedSources.bank && connectedBanks.length > 0 && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: connectedSources.email ? '8px' : '0' }}>
                ...
                <span style={{ color: '#fff', fontSize: '14px' }}>
                  {connectedBanks.length} bank{connectedBanks.length !== 1 ? 's' : ''}: {connectedBanks.join(', ')}
                </span>
              </div>
            )}
  ...
  const FoundScreen = () => {
    ...
    // Split NEW subs by complete/incomplete status
    const completeNewSubs = newlyAddedSubs.filter(s => s.complete && s.cycle && !s.needsName);
    const incompleteNewSubs = newlyAddedSubs.filter(s => !s.complete || !s.cycle || s.needsName);
    
    // Split EXISTING subs by complete/incomplete status
    const completeExistingSubs = existingSubs.filter(s => s.complete && s.cycle && !s.needsName);
    const incompleteExistingSubs = existingSubs.filter(s => !s.complete || !s.cycle || s.needsName);
    
    // All incomplete (both new and existing go to needs attention)
    const allIncompleteSubs = [...incompleteNewSubs, ...incompleteExistingSubs];
    
    // Include free trials in the display
    const displayFreeTrials = freeTrials.filter(t => selectedSubs.includes(t.id) || true); // Show all trials
    ...
    return (
      <div style={{
        ...
        <div style={{ padding: '0 20px' }}>
          ...
          {/* Free Trials section (if email connected) */}
          {connectedSources.email && displayFreeTrials.length > 0 && (
            <div style={{ marginBottom: '24px' }}>
              <div style={{ 
                display: 'flex', 
                alignItems: 'center', 
                ...
                <span style={{ fontSize: '14px', fontWeight: 600, color: '#F5A623' }}>
                  Free trials ({displayFreeTrials.length})
                </span>
              </div>
              {displayFreeTrials.map(trial => (
                <div
                  key={trial.id}
                  style={{
                    display: 'flex',
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: '13px', color: '#F5A623', fontWeight: 500 }}>
                      Trial ends in {trial.expiresIn} days → Then ${trial.amount}/mo
                    </div>
                  </div>
                  <button
                    onClick={() => {}}
                  ...
                  >
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="#F5A623">
                      <path d="M18 6L6 18M6 6L18 18" stroke="#F5A623" strokeWidth="2" strokeLinecap="round"/>
                    </svg>
                  </button>
                </div>
              ))}
          ...
          {/* Just added - COMPLETE new subs only */}
          {completeNewSubs.length > 0 && (
            <div style={{ marginBottom: '24px' }}>
              <div style={{ 
                display: 'flex', 
                alignItems: 'center', 
                ...
                <span style={{ fontSize: '14px', fontWeight: 600, color: '#0095FF' }}>
                  Just added ({completeNewSubs.length})
                </span>
              </div>
              {(showAllNew ? completeNewSubs : completeNewSubs.slice(0, 3)).map(sub => (
                <SubscriptionCardWithRemove key={sub.id} sub={sub} isNew />
              ))}
              {completeNewSubs.length > 3 && (
                <button
                ...
                >
                  {showAllNew ? 'Show less' : `Show ${completeNewSubs.length - 3} more`}
                </button>
              )}
            </div>
          )}

          {/* Ready to go - existing complete subs */}
          ...
          {/* Incomplete subscriptions - includes both new and existing that need attention */}
          {allIncompleteSubs.length > 0 && (
            <div style={{ marginBottom: '24px' }}>
              <div style={{ 
                display: 'flex', 
                alignItems: 'center', 
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <span style={{ fontSize: '14px', fontWeight: 600, color: '#F5A623' }}>
                    Needs attention ({allIncompleteSubs.length})
                  </span>
                </div>
    ...
    <div style={{
      ...
      <div style={{ padding: '40px 20px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <button
            onClick={() => {
              // Mock: Add some subscriptions found from email
              const emailSubs = [
                { id: 201, name: 'Adobe Creative Cloud', icon: '🎨', amount: 54.99, cycle: 'Monthly', nextRenewal: '5 Feb 2026', complete: true, category: 'Productivity', isNew: true, source: 'email' },
              ];
              setSubscriptions(prev => [...prev, ...emailSubs]);
              setSelectedSubs(prev => [...prev, ...emailSubs.map(s => s.id)]);
